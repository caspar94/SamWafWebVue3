<template>
  <div class="access-config">
    ...
    <t-form-item :label="t('page.host.access.mode')">
      <t-tooltip :content="t('page.host.access.mode_tips')" placement="top" :overlay-style="{ width: '380px' }" show-arrow>
    ...
    <t-form-item :label="t('page.host.access.exclude_paths')">
      <t-tooltip
        :content="t('page.host.access.exclude_paths_tips')"
        placement="top"
      ...
      >
        <t-textarea
          ...
          :placeholder="t('page.host.access.exclude_paths_ph')"
    ...
    <t-form-item :label="t('page.host.access.allow_ip_group')">
      <t-tooltip
        :content="t('page.host.access.allow_ip_group_tips')"
        placement="top"
        :overlay-style="{ width: '360px' }"
        show-arrow
      >
        <t-select
          v-model="local.allow_ip_group_code"
          clearable
          :style="{ width: '480px' }"
    ...
    <t-form-item :label="t('page.host.access.require_otp')">
      <t-tooltip
        :content="t('page.host.access.require_otp_tips')"
        placement="top"
        :overlay-style="{ width: '360px' }"
        show-arrow
      >
    ...
    <t-form-item :label="t('page.host.access.unauth_action')">
      <t-tooltip
        :content="t('page.host.access.unauth_action_tips')"
        placement="top"
        :overlay-style="{ width: '380px' }"
        show-arrow
      >
...
const local = ref<Record<string, any>>(JSON.parse(JSON.stringify(props.accessConfig)));
const ipGroups = ref<Record<string, any>[]>([]);

const cacheConflict = computed(() => props.cacheEnabled && local.value.mode !== '2');

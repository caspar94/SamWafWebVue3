<template>
  <div ref="pageRoot" class="vpconfig-page">
    <!-- 顶栏：页面标题 + 常驻「重启管理端」。
...
const sslStatusLoaded = ref(false);
const transportUpstreamTls = computed(
  () => sslStatusLoaded.value && window.location.protocol === 'https:' && !sslFormData.ssl_enable,
);
const transportSecure = computed(() => window.location.protocol === 'https:' && !transportUpstreamTls.value);
...
const transportStatusTips = computed(() => {
  if (transportUpstreamTls.value) return t('page.vpconfig.transport_status_upstream_tips');
...
const pickedAccessMode = ref('http');
const accessTouched = ref(false);
...
const savedAccessMode = computed(() => {
  if (!sslFormData.ssl_enable) return 'http';
  return sslForceHttpsFormData.force_https ? 'only' : 'https';
});
const accessDirty = computed(() => pickedAccessMode.value !== savedAccessMode.value);
...
function pickAccessMode(mode: string) {
  pickedAccessMode.value = mode;
  accessTouched.value = true;
}
...
watch(
  savedAccessMode,
  (val) => {
    if (!accessTouched.value) pickedAccessMode.value = val;
  },
  { immediate: true },
);
...
async function doSaveAccessMode(target: string) {
  const enable = target !== 'http';
  const force = target === 'only';
  const needEnable = enable !== !!sslFormData.ssl_enable;
  const needForce = force !== !!sslForceHttpsFormData.force_https;
  ...
  const steps = (enable ? [stepEnable, stepForce] : [stepForce, stepEnable]).filter(Boolean) as Array<() => Promise<any>>;
  ...
  try {
    for (const step of steps) {
      ...
      if (!res || res.code !== 0) {
        MessagePlugin.error(res?.msg || t('common.tips.save_failed'));
        if (done > 0) MessagePlugin.warning(t('page.vpconfig.access_save_partial'));
        return;
      }
      done += 1;
    }
    // 全部成功才认为"用户想要的状态已经落盘"，此时才让待保存值跟随后端
    accessTouched.value = false;
    MessagePlugin.success(t('common.tips.save_success'));
    markRestartPending();
  } catch (error) {
...
function handleSaveAccessMode() {
  if (!accessDirty.value) return;
  const target = pickedAccessMode.value;
  if (target !== 'http' && !certUsable.value) {
    const why =
      certSource.value === 'none'
        ? t('page.vpconfig.access_save_no_cert')
        : t('page.vpconfig.access_save_bad_cert', { reason: certProblem.value });
    const tail =
      target === 'only' ? t('page.vpconfig.access_save_tail_only') : t('page.vpconfig.access_save_tail_https');
    const dia = DialogPlugin.confirm({
      header: t('page.vpconfig.access_save_confirm_title'),
      body: `${why}

${tail}

${t('page.vpconfig.access_save_tail_common')}`,
      confirmBtn: { content: t('page.vpconfig.access_save_anyway'), theme: 'warning' },
      onConfirm: () => {
        dia.hide();
        doSaveAccessMode(target);
      },
    });
    return;
  }
...
const certUsable = computed(() => certSource.value !== 'none' && !certProblem.value);
const certStepDone = computed(() => pickedAccessMode.value !== 'http' && certUsable.value);
const certStepClass = computed(() => {
  if (pickedAccessMode.value === 'http') return 'step__n--todo';
  return certUsable.value ? 'step__n--done' : '';
});
const certStepDesc = computed(() => {
  if (pickedAccessMode.value === 'http') return t('page.vpconfig.access_step2_desc_http');
  if (certUsable.value) return t('page.vpconfig.access_step2_desc_ok');
  return t('page.vpconfig.access_step2_desc_need', { mode: accessModeLabel(pickedAccessMode.value) });
});
// 顶部说明里的条目。选的都是实际会被问到的，不是把文档抄一遍：
// 「浏览器报不安全」「改完没生效」「打不开了怎么救」占了这一节问题的绝大多数
const accessHelpItems = computed(() => [
  { k: t('page.vpconfig.access_help_k_save'), v: t('page.vpconfig.access_help_v_save'), tone: 'brand' },
  { k: t('page.vpconfig.access_help_k_insecure'), v: t('page.vpconfig.access_help_v_insecure') },
  { k: t('page.vpconfig.access_help_k_pick'), v: t('page.vpconfig.access_help_v_pick') },
  { k: t('page.vpconfig.access_help_k_locked'), v: t('page.vpconfig.access_help_v_locked'), tone: 'danger' },
  { k: t('page.vpconfig.access_help_k_local'), v: t('page.vpconfig.access_help_v_local') },
  { k: t('page.vpconfig.access_help_k_cdn'), v: t('page.vpconfig.access_help_v_cdn') },
]);

const certProblem = computed(() => {
  if (certSource.value === 'none') return '';
  return localCert.value?.cert_usable === false ? localCert.value?.cert_problem || '' : '';
});
...
function handleRestartManager(force?: boolean) {
  ...
  restartManagerApi({ force: !!force })
    .then((res) => {
      if (res.code === 0) {
        ...
        const dia = DialogPlugin.confirm({
          header: t('page.vpconfig.restart_blocked_title'),
          body: `${res.msg || t('page.vpconfig.restart_failed')}

${t('page.vpconfig.restart_blocked_tips')}`,
          confirmBtn: { content: t('page.vpconfig.restart_anyway'), theme: 'danger' },
          onConfirm: () => {
            dia.hide();
            handleRestartManager(true);
          },
        });
      } else {
        MessagePlugin.closeAll();
...
const sections = computed(() => [
  { id: 'vp-sec-ip', label: t('page.vpconfig.ip_whitelist'), show: true, dirty: false },
  { id: 'vp-sec-proxy', label: t('page.vpconfig.trusted_proxies_title'), show: true, dirty: false },
  { id: 'vp-sec-cors', label: t('page.vpconfig.cors_title'), show: true, dirty: false },
  { id: 'vp-sec-domain', label: t('page.vpconfig.domain_whitelist_title'), show: true, dirty: false },
  { id: 'vp-sec-access', label: t('page.vpconfig.access_cert_title'), show: true, dirty: accessDirty.value || restartPending.value },
  { id: 'vp-sec-entry', label: t('page.vpconfig.security_entry_title'), show: true, dirty: false },
  { id: 'vp-sec-notice', label: t('page.vpconfig.notice_title_title'), show: true, dirty: false },
]);
const visibleSections = computed(() => sections.value.filter((s) => s.show));
const activeSection = ref('');
const activeSectionLabel = computed(() => visibleSections.value.find((s) => s.id === activeSection.value)?.label || '');
// 顶栏是吸顶的，锚点跳过去若不留出这段高度，小节标题会被顶栏盖住
const stickyTop = ref(0);
const anchorOffset = computed(() => stickyTop.value + 64);
const sectionStyle = computed(() => ({ scrollMarginTop: `${anchorOffset.value}px` }));

const pageRoot = ref<HTMLElement | null>(null);
let spyScroller: HTMLElement | null = null;
...
// 这页不自建滚动容器：真正滚动的是布局的 .app-content。
...
function updateActiveSection() {
  ...
  if (!list.length || spyLock) return;
  const line = anchorOffset.value + 12;
  ...
  if (spyScroller && spyScroller.scrollTop + spyScroller.clientHeight >= spyScroller.scrollHeight - 4) {
...
watch(visibleSections, (list) => {
  if (activeSection.value && !list.some((s) => s.id === activeSection.value)) {
    activeSection.value = list.length ? list[list.length - 1].id : '';
  }
});

onMounted(() => {
  spyScroller = (pageRoot.value?.closest('.app-content') as HTMLElement) || findScrollParent(pageRoot.value);
  spyHandler = () => {
    if (spyRaf) return;
...
onUnmounted(() => {
  if (spyHandler) {
    (spyScroller || window).removeEventListener('scroll', spyHandler);
...
.vpconfig-nav__item:hover {
  background: var(--td-brand-color-light);
  color: var(--td-text-color-primary);
}

.vpconfig-nav__item.is-active {
  background: var(--td-brand-color-light);
...
.vpconfig-nav__item.is-dirty .vpconfig-nav__dot {
  visibility: visible;
}
